...
class ScrumCardDataHandler {
  ...
  Future<List<ScrumCard>> getAll() async {
    try {
      ...
      List<ScrumCard> scrumCards = List<ScrumCard>.from(
        jsonArray.map((jsonObject) => ScrumCard.fromMap(jsonObject)) // Turn json array into Gallery Item objects
        );
  ...
  Future<ScrumCard> create(ScrumCard scrumCard) async {
    ...
    if(responseBody.isNotEmpty) {
      try {
        ScrumCard createdScrumCard = ScrumCard.fromJson(responseBody); // Should return the created model
    ...
    if(uri == null) {
      throw MissingUriException(message: "Missing URI for created ${(ScrumCard).toString()}");
    }
  ...
  Future<bool> edit(ScrumCard scrumCardChanges) async {
    ...
    await _apiClient.put("$_entityContext/${scrumCardChanges.id}", jsonBody); // Saving response for easy extension refactor of the method
    return true;
  }
